...
app.get('/admin/properties/property-detail/:id', (req, res, next) => {
        ...
        const response = axios.get(`https://backend.sistemacic.com/api/properties/${propertyId}?populate=*`, {}).then((response) => { 
                property = response.data.data.attributes;
                
                (property?.photos?.data !== null
                        ? imageUrl = `https://backend.sistemacic.com${property?.photos?.data[0]?.attributes?.url}`
                        : imageUrl = 'https://backend.sistemacic.com/uploads/no_image_default_0e6727a941.jpg')
        ...
        .replace('https://backend.sistemacic.com/uploads/blue_logo_d00dd4ed3a.png', imageUrl)
                return res.send(htmlData);
        })
});
...
app.get('/home/search/property-detail/:id', (req, res, next) => {
        ...
        .replace('Venta y Alquiler de inmuebles y propiedades', `${property?.tipoPropiedad} - ${property?.provincia} - ${property?.canton} - ${property?.moneda}${property?.precio}`)
        .replace('__META_DESCRIPTION__', property?.tipoPropiedad)
        .replace('https://backend.sistemacic.com/uploads/blue_logo_d00dd4ed3a.png', `https://backend.sistemacic.com${property?.photos.data[0].attributes.url}`)
...
app.get('/home/portfolio/share-portfolio/:id', (req, res, next) => {
        ...
        const response = axios.get(`https://backend.sistemacic.com/api/portafolios/${portafolioId}?populate=*`, {}).then((response) => { 
...
app.get('/home/visiter-contact', (req, res, next) => {
        fs.readFile(indexPath, 'utf8', (err, htmlData) => {
                if (err) {
                        console.error('Error leyendo el archivo index', err);
                        return res.status(404).end()
...
app.get('/admin/buttons', (req, res, next) => {
        fs.readFile(indexPath, 'utf8', (err, htmlData) => {
                if (err) {
                        console.error('Error leyendo el archivo index', err);
                        return res.status(404).end()
...
app.get('/home/visiter-search', (req, res, next) => {
        fs.readFile(indexPath, 'utf8', (err, htmlData) => {
                if (err) {
                        console.error('Error leyendo el archivo index', err);
                        return res.status(404).end()
...
app.get('/home/video-player', (req, res, next) => {
        fs.readFile(indexPath, 'utf8', (err, htmlData) => {
                if (err) {
                        console.error('Error leyendo el archivo index', err);
                        return res.status(404).end()
