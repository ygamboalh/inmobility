  ...
  return (
    <div className="profile-photo">
      <form onSubmit={formik.handleSubmit}>
        <input
          onChange={(event) => {
            formik.setFieldValue("files", event.target.value);
        ...
        {formik?.errors?.files &&
          ...
          formik?.errors?.files}
        <button type="submit" className="mt-4 text-xl">
          asasas
        </button>
      </form>
    </div>
    /*{/* <div className="relative w-24 h-24 border rounded-full overflow-hidden">
            <img src="ruta_de_la_imagen" alt="Foto de perfil" className="w-full h-full object-cover"/>
            <button onClick={handleAPI} className="absolute bottom-0 right-0 bg-white rounded-full p-1">
